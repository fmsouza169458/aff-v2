...
class AFFStrategy(FedAvg):
    ...
    def configure_fit(self, server_round, parameters, client_manager):
        ...
        heterogeneity_factor = self.compute_model_heterogeneity()
        if self.latest_accuracy is not None:
            print("A")
            new_client_count = self.aff.update(self.latest_accuracy, heterogeneity=heterogeneity_factor)
        else:
            print("B")
    ...
    def aggregate_fit(self, server_round: int, results, failures):
        ...
        for _, res in results:
            params_size = sum(arr.nbytes for arr in self.parameters_to_ndarrays(res.parameters))
    ...
    def evaluate(
        ...
        with open("results_last_executed_aff_with_het.json", "w") as f:
